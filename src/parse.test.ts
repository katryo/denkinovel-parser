...
describe('When tags has syntax errors, an exception should be thrown', (): void => {
  it.each([
    ...
    expect(() => {
      parse(`${incorrectBracket}paragraph`);
    }).toThrow(Error);
  });
  ...
  it.each([
    ...
    expect(() => {
      parse(`${bracket1}${bracket2}paragraph`);
    }).toThrow(Error);
  });
});

describe('When tag is incorrect, an exception should be thrown', (): void => {
  it.each(['music', 'sound', 'filter', 'image', 'bg', 'key'])('value does not exist (%s)', (keyName): void => {
    expect(() => {
      parse(`[${keyName}]paragraph`);
    }).toThrow(Error);
  });

  it.each(['music', 'sound', 'filter', 'image', 'bg', 'key'])('multiple values (%s)', (keyName): void => {
    expect(() => {
      parse(`[${keyName} value1 value2]paragraph`);
    }).toThrow(Error);
  });

  it.each([
    ['one and zero', '[bg value]paragraph1[sound]paragraph2'],
    ['one and two', '[bg value]paragraph1[sound value1 value2]paragraph2'],
  ])('Correct value and incorrect value (%s)', (title, sectiontags): void => {
    expect(() => {
      parse(`[bg value]paragraph1[sound]paragraph2`);
    }).toThrow(Error);
  });
});

describe('When tags are correct,', (): void => {
  describe('but key names are incorrect, an exception should thorown', (): void => {
    it.each(['unknown', 'mmusic', 'soundd', 'ffilter', 'imagee', 'bgg'])('%s', (incorrectKeyName): void => {
      expect(() => {
        parse(`[${incorrectKeyName} value]paragraph`);
      }).toThrow(Error);
    });
    ...
    it.each([
      ...
      expect(() => {
        parse(`[${incorrectKeyName} value]paragraph1[${correctKeyname} value]paragraph2`);
      }).toThrow(Error);
    });
  });
